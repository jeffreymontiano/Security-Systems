...
| Module | Capabilities |
|---|---|
| **Security Operations Dashboard** | KPI cards, pie/column charts, trend filters; operational records — **Daily Manning** (Deployment Status), **Site Status** (Site Condition), **Site Manning Status** (Complete / Incomplete / No Guards), **Patrol Video** (Video Patrol Status: Complete / Incomplete), Visitor Count, Vehicle Count. Guard names are **picked from the 201 File** (`Full Name — Employee No`), and all three status lists are admin-maintainable from Manage Lists |
