...
- **A GRACE THRESHOLD absorbs sub-shift rounding, and it applies at Cap 2 —
  never day- or site-level.** A completed shift worked within `graceMinutes` of
  its full duty counts as a full shift, so an 8–14 min late clock-in stops
  reading as a misleading "Under-manned" LESS the client is credited for — which
  is what the DTR (flat 12 h band) and the roster-anchored payslip already do.
  Applied in `workedOn()`: `target = units × dutyHours`; if
  `0 < (target − v.hours) ≤ units × graceHours` the guard counts `target`, else
  `v.hours`. Grace scales per covered shift, so a straight duty gets
  `units × grace`.
  - **It is STRUCTURALLY unable to absorb real under-manning.** Grace only ever
    adjusts a `v` in the `worked` map, and that map is populated **exclusively
    from completed punch pairs**. A missing guard, a rest day, a closed day and a
    held (incomplete) pair have **no entry** — grace never sees them, so their
    full 12 h LESS stands. A shift short by MORE than the per-shift allowance
    fails the threshold and keeps its partial LESS. So grace can only ever close
    a small gap on a shift that was genuinely worked; it can never manufacture
    hours for one that was not. This is the money-path safety property.
  - **`graceMinutes` is the SAME value payroll reads** —
    `payroll_statutory_config.pay_rules.graceMinutes` (seeded 15) — loaded once
    in the compute route and passed to the pure engine as an option, like the
    closed-day config. **Absent or ≤ 0 resolves to 0 hours = the previous
    behaviour exactly**, so an install that never set it is byte-identical.
  - **Measured on Aug 16–31, nine sites**: grace=15 absorbs **1.18 h** of
    sub-shift rounding total (BBGC 0.4→0, Hatchery 0.09→0, and the fractional
    tails of Burot/BFC Base 1/RH) while **genuine under-manning moves by exactly
    zero** — Saluyot 72→72, and every whole-shift 12 h chunk preserved.
    **Brookdale's Aug 28 stays a LESS**: SG Tuscano worked 11.6 h, 24 min short —
    beyond the 15 min grace, so it is correctly declined, not rounding noise.
