...
| **Shift Scheduling** | Shift templates and per-day roster; `crossesMidnight` derived from the times; **`shiftKind` (Day / Night / Straight Duty)** stated on the template and snapshotted onto **every** assignment; a **roster legend derived from the templates**, so a new shift type appears with no code change; explicit rest days that restore the prior shift — and its kind — when removed |
